...
2. Open the saved thread, open Goal, create a non-empty objective with a Token budget, and close/reopen the menu.
...
5. Edit the objective and Token budget and verify usage resets according to official Goal replacement semantics; edit again, leave the budget empty, and verify the limit is removed while the Goal remains.
...
- Edit applies the entered Token budget or removes the limit when it is empty, while editing a complete or budget-limited Goal reactivates it.

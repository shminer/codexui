...
const SIDE_CONVERSATION_DEVELOPER_INSTRUCTIONS = `You are in a side conversation, not the main thread.
...
export type StartedSideConversation = {
  threadId: string
}
...
export async function startSideConversation(
  ...
  if (!normalizedParentThreadId) {
    throw new Error('Side conversation requires a parent thread')
  }
  ...
  try {
    ...
    await callRpc('thread/inject_items', {
      ...
      items: [{
        type: 'message',
        role: 'user',
        content: [{ type: 'input_text', text: SIDE_CONVERSATION_BOUNDARY_PROMPT }],
      }],
    })
...
export async function getCurrentModelConfig(): Promise<CurrentModelConfig> {
  ...
  const developerInstructions = typeof payload.config.developer_instructions === 'string'
    ? payload.config.developer_instructions
    : ''
  return { model, providerId, upstreamCatalogProviderIds, reasoningEffort, speedMode, developerInstructions }
}

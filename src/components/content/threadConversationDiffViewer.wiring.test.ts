...
describe('ThreadConversation mobile diff viewer wiring', () => {
  it('keeps the close button outside the diff toolbar at every viewport width', async () => {
    const source = await readFile(new URL('./ThreadConversation.vue', import.meta.url), 'utf8')

    expect(source).not.toContain('<button v-if="!isMobile" class="image-modal-close diff-viewer-close"')
    expect(source).toMatch(/<\/section>\s*<button\s+class="image-modal-close diff-viewer-close diff-viewer-mobile-close"/u)

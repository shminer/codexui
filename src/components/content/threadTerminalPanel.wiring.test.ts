...
describe('ThreadTerminalPanel floating terminal wiring', () => {
  it('keeps the terminal draggable, resizable, and equipped with one-shot shortcuts', async () => {
    ...
    expect(source).toContain('class="thread-terminal-drag-handle"')
    expect(source).toContain('@pointerdown.stop="onTerminalHeaderPointerDown"')
    expect(source).toMatch(/\.thread-terminal-drag-handle\s*\{[\s\S]*touch-action:\s*none/u)
